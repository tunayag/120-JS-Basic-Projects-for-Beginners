...
function timer(timeLeft) {
    ...
    timeEl = setInterval(() => {
        if (timeLeft > 0) {
            ...
            return timeLeftEl.innerText = timeLeft;
...
function initTyping() {
    let characters = textEl.querySelectorAll("span");
    let typedCharacter = inputEl.value.split("")[characterIndex];

    if (characterIndex < characters.length - 1 && timeLeft > 1) {
        ...
        cpmEl.textContent = characterIndex - mistakes;
...
function getText() {
    ...
    randomText = paragraphs[ranIndex];
    textEl.innerHTML = "";
    randomText.split("").forEach((spanEl) => {
        let spanTag = `<span>${spanEl}</span>`
        textEl.innerHTML += spanTag;
    });
    textEl.querySelectorAll("span")[0].classList.add("active");
